...
export function ReplyThread({ commentId, level }: { commentId: string, level: number }) {
    ...
    <div className="space-y-2">
      ...
      {!!children.length && (
        <div className="ml-10 border-l pl-4 border-gray-200">
          {children.map(id => <ReplyThread key={id} commentId={id} level={level+1} />)}
          {hasMore && (
            <button className="text-sm text-brand-700 hover:text-brand-800" onClick={()=>setExpanded(true)}>Show more replies</button>
          )}
        </div>
      )}
    </div>
